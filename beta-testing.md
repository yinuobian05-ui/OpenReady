...
General metrics snapshot time: 2026-08-14 12:31 UTC+8. Release-state fields and the merged pull-request count were refreshed on 2026-08-15 at 11:23 UTC+8; other metrics below remain at the earlier snapshot. Metrics can lag and must not be inferred forward.
...
| Public issues / pull requests | 0 user issues; 0 external pull requests; 6 merged maintainer pull requests |
...
| Published releases | GitHub v0.1.0, v0.1.1, v0.1.2, and v0.2.0; npm `latest` is v0.2.0 |
| Maintenance checks | Main CI for `a8d41f8` passed on Node.js 20, 22, and 24. The v0.2.0 release validation reports 57 tests, 56 passed, 0 failed, and 1 platform-limited skip; its package dry-run listed 23 intended files, its source self-scan returned `READY`, and its direct synthetic demo completed with the expected result and cleanup. Public CI for pull request #7 and merged `main` commit `c360d368` passed on Ubuntu with Node.js 20, 22, and 24, and on macOS and Windows with Node.js 20. The public v0.2.0 package contains the same 23 reviewed files and expected tarball hash. A fresh unauthenticated registry install reported version `0.2.0`, completed the synthetic demo with 4 blockers, 5 warnings, and 1 info, removed its temporary files, returned `READY` with 0 blockers and 0 warnings for the installed-package scan, and produced a zero-vulnerability npm audit. |
